...
export function MultiSelectDropdown({
  ...
  return (
    <div ref={containerRef} className={cn("relative", className)}>
      ...
      >
        {icon && <span className={cn("shrink-0", hasSelection ? "text-accent" : "text-text-muted")}>{icon}</span>}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-1 w-full min-w-[220px] bg-surface border border-border rounded-(--radius-md) p-1 z-20 shadow-(--shadow-md)">
          {/* All option */}
          <label
            className={cn(
          ...
          <div className="max-h-64 overflow-y-auto">
            {hasGroups ? (
              groups.map((group) => (
                <div key={group.key}>
                  {group.key && (
                    <div className="px-2 pt-2 pb-1 text-ui text-text-muted font-medium uppercase tracking-wider">
                      {group.key}
                    </div>
                  )}
