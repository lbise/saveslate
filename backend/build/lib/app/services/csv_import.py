...
TRANSFORMABLE_FIELDS = {"description", "category", "currency"}


def _get_transform_key(transform: dict[str, Any], camel: str, snake: str) -> Any:
    return transform.get(camel, transform.get(snake))


def _normalize_regex_pattern(pattern: str) -> str:
    """Accept JS-style named groups (`(?<name>...)`) in backend regexes."""
    return re.sub(r"\(\?<([A-Za-z_][A-Za-z0-9_]*)>", r"(?P<\1>", pattern)


def apply_field_transforms(row: ParsedRow, transforms: list[dict[str, Any]]) -> None:
    """Apply configured field transforms to a parsed row in sequence."""
    any_transform_matched = False

    for transform in transforms:
        match_pattern = _get_transform_key(transform, "matchPattern", "match_pattern") or ""
        extract_pattern = _get_transform_key(transform, "extractPattern", "extract_pattern") or ""
        if not match_pattern or not extract_pattern:
            continue

        source_field = _get_transform_key(transform, "sourceField", "source_field") or ""
        target_field = _get_transform_key(transform, "targetField", "target_field") or ""
        if source_field not in TRANSFORMABLE_FIELDS or target_field not in TRANSFORMABLE_FIELDS:
            continue

        source_value = getattr(row, source_field, None)
        if not isinstance(source_value, str) or not source_value:
            continue

        label = transform.get("label") or "unnamed"

        try:
            match_re = re.compile(_normalize_regex_pattern(match_pattern), re.IGNORECASE)
            if not match_re.search(source_value):
                continue

            any_transform_matched = True

            extract_re = re.compile(_normalize_regex_pattern(extract_pattern))
            match = extract_re.search(source_value)
            groups = match.groupdict() if match else {}

            if groups:
                replacement = transform.get("replacement") or ""
                if replacement:
                    result = re.sub(
                        r"\{\{(\w+)\}\}",
                        lambda replacement_match: groups.get(replacement_match.group(1), "") or "",
                        replacement,
                    )
                else:
                    result = " ".join(value for value in groups.values() if value)

                if target_field == "description":
                    row.description = result
                else:
                    setattr(row, target_field, result or None)
            else:
                row.errors.append(
                    f'Transform "{label}": extract pattern has no named group matches'
                )
        except re.error:
            row.errors.append(f'Transform "{label}": invalid regex')

    if not any_transform_matched and transforms:
        has_content = any(
            isinstance(getattr(row, _get_transform_key(transform, "sourceField", "source_field") or "", None), str)
            and getattr(row, _get_transform_key(transform, "sourceField", "source_field") or "", "").strip()
            for transform in transforms
        )
        if has_content:
            row.errors.append("No transform rule matched this row")
...
def apply_parser(
    ...
    transforms: list[dict[str, Any]] = parser_config.get("transforms") or []
    ...
    for row in rows:
        ...
        if transforms:
            apply_field_transforms(parsed[-1], transforms)
